...
## 追加セクション A: CMS・ホームページ管理 (2026-03-13 追加)

> レビューにより欠落が判明した機能群。SiteSettings, HeroBanner, BannerAd, HomepageCustomBlock, ExternalLink のテスト。

**前提**: 管理者 (`superuser` または `owner`) でログイン済み

### A.1 サイト設定 (SiteSettings)

```
テストURL: /admin/booking/sitesettings/1/change/
```

- [ ] 1. 管理画面 → ページ設定 → メインサイト設定 を開く
- [ ] 2. 「店舗カード表示」をOFF → 保存 → トップページ (`/`) で店舗カードが非表示になること
- [ ] 3. 「占い師カード表示」をOFF → 保存 → 占い師カードが非表示
- [ ] 4. 各カード表示をON に戻す → トップページで表示復帰
- [ ] 5. 「ヒーローバナー表示」をOFF → トップページのスライダー非表示
- [ ] 6. 「予約ランキング表示」をOFF → ランキングセクション非表示
- [ ] 7. スタッフ呼称を「セラピスト」に変更 → 管理画面・フロントで反映
- [ ] 8. スタッフ呼称（多言語）に `{"en": "Therapist"}` を設定 → `?lang=en` で確認

### A.2 ヒーローバナー管理

```
テストURL: /admin/booking/herobanner/
```

- [ ] 1. ヒーローバナーを追加 → タイトル、画像、表示位置を設定
- [ ] 2. リンク種別「店舗」→ リンク先店舗を選択 → 保存 → クリックで遷移確認
- [ ] 3. リンク種別「カスタムURL」→ 外部URL → 新タブで開くこと
- [ ] 4. 並び順変更 → バナースライダーの表示順が変わること
- [ ] 5. 「公開」をOFF → バナーが非表示になること

### A.3 法定ページ

- [ ] 1. `/booking/privacy/` にアクセス → デフォルトテンプレート表示
- [ ] 2. SiteSettings で `privacy_policy_html` にカスタムHTMLを入力 → 保存 → 反映確認
- [ ] 3. `/booking/tokushoho/` で特商法表記の表示確認

### A.4 バナー広告・カスタムHTMLブロック

- [ ] 1. バナー広告を追加（表示位置設定）→ トップページで表示確認
- [ ] 2. カスタムHTMLブロックを追加 → 該当位置に表示確認
- [ ] 3. XSSテスト: `<script>alert('xss')</script>` がサニタイズされるか確認

---

## 追加セクション B: マイページ機能 (2026-03-13 追加)

**前提**: スタッフユーザーでログイン済み

### B.1 マイページ表示

- [ ] 1. `/booking/mypage/` にアクセス → ログイン中スタッフのマイページ表示
- [ ] 2. 自分の予約一覧が表示されること
- [ ] 3. カレンダーリンク → `/booking/mypage/<pk>/calendar/` に遷移
- [ ] 4. カレンダーに予約が色付きで表示されること
- [ ] 5. 特定日クリック → 日次詳細に遷移

### B.2 予約編集・削除

- [ ] 1. 予約クリック → 編集画面 (`/booking/mypage/schedule/<pk>/`)
- [ ] 2. 備考を変更 → 保存 → 変更反映確認
- [ ] 3. 予約削除 → 削除確認 → 削除実行
- [ ] 4. 他人の予約にはアクセスできないこと (403/404)

### B.3 休日追加

- [ ] 1. マイページカレンダーから休日追加ボタン → 該当日に休日設定
- [ ] 2. 休日設定された日に予約が入らないこと確認

---

## 追加セクション C: 日付先行カレンダー予約 (2026-03-13 追加)

- [ ] 1. `/booking/date-calendar/` にアクセス → カレンダー表示
- [ ] 2. 日付クリック → 該当日の空きスタッフ表示
- [ ] 3. 特定日付URL (`/booking/date-calendar/<y>/<m>/<d>/`) → 正しい日のカレンダー表示
- [ ] 4. 空きコマ選択 → 予約確認画面に遷移

---

## 追加セクション D: 雇用契約・給与体系 (2026-03-13 追加)

### D.1 雇用契約 (EmploymentContract)

- [ ] 1. スタッフの雇用契約を作成（雇用形態: パート、時給: 1200円）
- [ ] 2. 各種手当（通勤・住宅・家族）を設定
- [ ] 3. 標準報酬月額を設定
- [ ] 4. 契約開始・終了日を設定

### D.2 給与体系 (SalaryStructure)

- [ ] 1. 店舗の給与体系を作成
- [ ] 2. 社会保険料率を設定 → 給与計算に反映されること
- [ ] 3. 割増率（残業1.25/深夜1.35/休日1.50）を設定
- [ ] 4. 給与計算アクション実行 → 設定した料率で正しく計算されること

---

## 追加セクション E: 顧客向け注文フロー (2026-03-13 追加)

- [ ] 1. `/booking/menu/<store_id>/` → メニュー一覧表示
- [ ] 2. カテゴリ別フィルタリング確認
- [ ] 3. `/api/menu` でJSON取得 → 商品一覧が正しいこと
- [ ] 4. 注文後 `/api/orders/status/` で注文ステータス確認
- [ ] 5. `POST /api/products/alternatives/` → 代替商品リスト取得

---

## 追加セクション F: 勤怠管理追加テスト (2026-03-13 追加)

### F.1 休憩打刻

- [ ] 1. 出勤打刻後、`stamp_type=break_start` で休憩開始
- [ ] 2. 出退勤ボードで「休憩中」ステータス表示
- [ ] 3. `stamp_type=break_end` で休憩終了 → 「出勤中」に戻ること

### F.2 ジオフェンスチェック

- [ ] 1. `AttendanceTOTPConfig` で `require_geo_check=True` + 座標・半径を設定
- [ ] 2. 正しい座標から打刻 → 成功
- [ ] 3. 範囲外から打刻 → 「店舗の範囲外です」エラー

### F.3 WorkAttendance 自動生成

- [ ] 1. PIN打刻（出勤）→ `WorkAttendance` の `clock_in`, `source='pin'` 確認
- [ ] 2. QR打刻（退勤）→ `clock_out`, `source='qr'` 確認
- [ ] 3. 同日複数打刻 → 既存レコード更新（新規作成されないこと）

---

## 追加セクション G: ロール別アクセス制御追加テスト (2026-03-13 追加)

### G.1 owner ロール

- [ ] 1. `is_owner=True` のスタッフでログイン
- [ ] 2. 全モデルが管理画面に表示されること
- [ ] 3. CRUD操作すべて可能なこと

### G.2 AdminMenuConfig カスタマイズ

- [ ] 1. システム → メニュー設定 → `manager` ロールの許可モデルを変更
- [ ] 2. `manager` ロールでログイン → 変更が反映されること
- [ ] 3. 空リストに設定 → デフォルトにフォールバックすること

---

## 追加セクション H: 追加APIエンドポイント (2026-03-13 追加)

| # | エンドポイント | メソッド | 認証 | 期待レスポンス | 確認 |
|---|--------------|--------|------|-------------|------|
| 1 | `/api/endTime` | GET | なし | `{time: ISO8601}` | [ ] |
| 2 | `/api/currentTime` | GET | なし | `{time: ISO8601}` | [ ] |
| 3 | `/api/reservation/<pk>/` | GET | なし | `{startTime, endTime}` | [ ] |
| 4 | `/api/menu` | GET | なし | 200, 商品リスト | [ ] |
| 5 | `/api/products/alternatives/` | POST | なし | 200, 代替商品 | [ ] |
| 6 | `/api/orders/create/` | POST | なし | 201, 注文作成 | [ ] |
| 7 | `/api/orders/status/` | GET | なし | 200, ステータス | [ ] |
| 8 | `/api/staff/orders/served/` | POST | Session | 200, 提供完了 | [ ] |
| 9 | `/api/stock/inbound/apply/` | POST | Session | 200, 入庫適用 | [ ] |
| 10 | `/api/table/<uuid>/orders/status/` | GET | なし | 200, テーブル注文 | [ ] |
