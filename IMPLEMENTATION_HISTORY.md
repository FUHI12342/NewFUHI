...
2. [技術スタック](#技術スタック)
3. [モデル一覧 (55モデル)](#モデル一覧)
4. [API エンドポイント一覧 (51アクティブ)](#apiエンドポイント一覧)
5. [実装機能履歴 (Task #24〜#36)](#実装機能履歴)
6. [コード解析・改修履歴 (2026-03-13)](#コード解析改修履歴)
7. [追加改修 (2026-03-16, Task #41〜#45)](#追加改修-2026-03-16)
8. [自動テスト一覧 (1041+7テスト)](#自動テスト一覧)
8. [手動テスト項目一覧 (HANDTEST.md)](#手動テスト項目一覧)
9. [ドキュメント一覧](#ドキュメント一覧)
10. [マイグレーション履歴 (65件)](#マイグレーション履歴)
...
- **CI/CD**: pytest (769 passed + 7 skipped), HANDTEST.md (29セクション)
...
全51アクティブルート (`booking/api_urls.py`) ※2件無効化中
...
| `/api/dashboard/visitor-forecast/` | GET | 来客予測 |
| `/api/dashboard/clv/` | GET | CLV分析 |
| `/api/dashboard/auto-order/` | GET | 自動発注推奨 |
| `/api/dashboard/external-data/` | GET | 外部データ連携 |
...
### 無効化中 (2) ※Gemini APIキー再発行まで
| パス | メソッド | 説明 | 状態 |
|------|--------|------|------|
| `/api/chat/admin/` | POST | AIチャット (管理者) | 一時停止 (レート制限20回/5分追加済) |
| `/api/chat/guide/` | POST | AIチャット (ガイド) | **廃止** (コストリスク対策) |
...
everything-claude-code 導入後、Django バックエンド (55モデル, 53 API, 776テスト) と IoT デバイスコード (MicroPython/CircuitPython, Pico 2W) を包括解析。

| 項目 | 結果 |
|------|------|
| 全テスト実行 | **769 passed, 7 skipped** (74.93秒) |
| Django モデル数 | 55 |
| APIエンドポイント数 | 51 アクティブ (2件無効化) |
...
| 5 | IoT Setup AP: ハードコードパスワード `"SETUP_PASSWORD"` | `pico_device/setup_ap.py` | **修正済 (MAC/UID動的生成)** |
| 6 | IoT API通信: HTTP (非HTTPS) | `config.py` | **修正済 (HTTPS化)** |
| 7 | WiFi credentials 平文コミット | `secrets.py` | **修正済 (template+.gitignore)** |
| 8 | Staff attendance_pin: 平文4桁PIN | `models.py` | **修正済 (make_passwordハッシュ)** |

**MEDIUM**
| # | 問題 | ファイル | 対応状況 |
|---|------|---------|---------|
| 9 | N+1クエリ (ダッシュボードAPI) | `views_restaurant_dashboard.py` | **修正済 (select_related)** |
| 10 | LINE通知リトライなし | `views.py` | **修正済 (指数バックオフ)** |
| 11 | IoT: Watchdogタイマー未実装 | `code.py` | **修正済 (WDT 60秒)** |
| 12 | IoT: DHT22センサープレースホルダー | `code.py` | **修正済 (SHT31/DHT20自動検出)** |
| 13 | IoT API レート制限なし | `views.py` | **修正済 (10req/min throttle)** |
...
#### Phase 2-3 修正内容 (2026-03-13 実施)

**IoTセキュリティ (6件)**
- `setup_ap.py`: ハードコードパスワード → MAC/UID基づく動的生成 (`FUHI-XXXXXX`)
- `config.py`: HTTP → HTTPS 通信
- `secrets_template.py`: 新規作成、`secrets.py` を `.gitignore` に追加
- `code.py`: WDT (60秒タイムアウト) 追加、ハング時自動リセット
- `dht_i2c.py`: プレースホルダー → SHT31/DHT20 I2Cセンサー自動検出実装
- `.gitignore`: IoTデバイス秘匿ファイルエントリ追加

**Djangoセキュリティ・信頼性 (3件)**
- `models.py`: `attendance_pin` を `make_password` / `check_password` でハッシュ化 (後方互換あり)
- `line_notify.py`: 指数バックオフ3回リトライ + HTTP 429 対応
- `views.py`: `IoTDeviceThrottle` (10req/min per device) レート制限

**パフォーマンス (N+1修正)**
- `StaffPerformanceAPIView`: N*2+1 → 2クエリ (annotate+aggregate最適化)
- `LowStockAlertAPIView`, `InsightsAPIView`, `CustomerFeedbackAPIView`: `select_related` 追加

**新機能 (4件)**
- `来客予測API` (`/api/dashboard/visitor-forecast/`): 曜日別移動平均 + 95%信頼区間 + スタッフ配置推奨
- `CLV分析API` (`/api/dashboard/clv/`): 顧客生涯価値計算 + 5セグメント (high/medium/low/at-risk/lost)
- `自動発注推奨API` (`/api/dashboard/auto-order/`): 消費速度 × 在庫 × リードタイム → 発注推奨量
- `外部データ連携API` (`/api/dashboard/external-data/`): OpenWeatherMap/Google Business Profile スケルトン

**マイグレーション**: `0065_attendance_pin_hash`

#### Phase 4 コスト対策 (2026-03-13 実施)

**Gemini APIコストリスク分析**
- GuideChatAPIView: 認証なし公開エンドポイント → 無制限呼出リスク → **完全廃止**
- AdminChatAPIView: レート制限なし → 管理者悪用リスク → **レート制限追加 + URL無効化**
- IoTEventの無制限蓄積（~4,320行/日/デバイス）→ 将来的にクリーンアップタスク追加予定

**修正内容**:
- `views_chat.py`: `GuideChatAPIView` クラス削除、`AdminChatAPIView` にキャッシュベースレート制限 (20回/5分) 追加
- `api_urls.py`: chat/admin URL コメントアウト、chat/guide URL+import 削除
- `base.html`: ガイドチャットウィジェット (`guide_chat_widget.js`) の読込を削除
- テスト: 776 passed → 769 passed + 7 skipped (チャットURL無効化によりスキップ)

#### ドキュメント整備 (2026-03-14)

**HANDTEST.md更新**: セクション28-29追加 (セキュリティ改修検証 + 来客予測・CLV・自動発注)
**Mermaid図作成**: `docs/diagrams/` に6枚のシステム構成図をPNG出力
- `system_architecture` — 全体アーキテクチャ (AWS EC2)
- `iot_flow` — IoTセンサー→アクションフロー
- `booking_flow` — 予約プロセス
- `pos_flow` — POS + キッチン + テーブル注文
- `dashboard_analytics` — 分析モジュール一覧
- `security_overview` — セキュリティレイヤー
...
| 通信方式 | 3-tier: Django HTTPS (主) / AWS MQTT (副) / Setup AP (緊急) |
| センサー | MQ-9 (CO), PIR (人感), DHT22/SHT31 (温湿度) |
| 送信間隔 | 20秒 |
...
### Task #41: 追加改修6点 — 税設定・EC/メニュー分離・店舗紹介・料金ラベル・権限別メニュー・PIN打刻昇格
- **実装内容**:
  - `TaxServiceCharge` モデル追加 — 店舗ごとの税率・サービス料・深夜料金管理
  - `Category.is_restaurant_menu` — EC商品/飲食メニューの分離フラグ
  - `Store.description`, `access_info`, `map_url` — 店舗紹介フィールド
  - `SiteSettings.price_label` — 料金ラベルカスタマイズ（「鑑定料」「料金」等）
  - `AdminMenuConfig` — ロール別の管理画面メニュー表示制御 (superuser のみ編集可)
  - PIN打刻 — `attendance_pin` ハッシュ化 + PIN打刻キオスク画面
- **マイグレーション**: `0066`〜`0070`
- **テスト**: 1041 passed + 7 skipped

### Task #42: お知らせ詳細ページ + WordPress風リッチテキストエディタ
- **実装内容**:
  - `Notice` モデル拡張 — `slug`, `is_published`, `thumbnail`, `created_at` フィールド追加
  - `NoticeAdmin` — TinyMCE CDN リッチテキストエディタ統合、公開/下書き管理
  - `NoticeListView` / `NoticeDetailView` — お知らせ一覧・詳細ページ（`/news/`, `/news/<slug>/`）
  - サイドバーリンク改善 — 内部ページリンク優先、「もっと見る」リンク追加
  - `notice` をページ設定グループに移動（よく使うため）
  - `context_processors.py` — `is_published=True` フィルタ追加
- **ファイル**: `models.py`, `admin.py`, `views.py`, `urls.py`, `admin_site.py`, `context_processors.py`, `base.html`, 新規テンプレート2件
- **マイグレーション**: `0071_notice_add_slug_published_thumbnail`

### Task #43: 在庫管理ダッシュボード
- **実装内容**:
  - `InventoryDashboardView` — 在庫一覧ダッシュボード（`/admin/inventory/`）
  - `StockInFormView` — 入荷登録フォーム（モーダルUI）
  - EC商品/飲食メニュー/全てのカテゴリフィルタ
  - 低在庫のみフィルタ、在庫ステータスバッジ（在庫あり/低在庫/在庫切れ）
  - KPI統計（商品数、低在庫数、在庫切れ数）
  - `inventory` サイドバーグループ有効化 + `StockMovement` 管理画面復活
- **ファイル**: 新規 `views_inventory.py`, `inventory_dashboard.html`, `project/urls.py`, `admin_site.py`

### Task #44: デモデータ画像生成
- **実装内容**:
  - `seed_mock_data` コマンドに Pillow ベースのプレースホルダー画像生成追加
  - カテゴリ別の背景色（シーシャ=紫, ドリンク=青, フード=オレンジ, 占い=ピンク, グッズ=緑）
  - 商品名 + カテゴリ名 + 価格テキスト入りの 400x300px PNG画像
  - 全26商品に画像自動割当
  - お知らせモックデータをリッチHTML形式に更新
- **ファイル**: `seed_mock_data.py`

### Task #45: PIN打刻 通信エラー修正
- **実装内容**:
  - `CSRF_TRUSTED_ORIGINS` 設定追加（Django 4.x HTTPS必須対応）
  - `ALLOWED_HOSTS` からデフォルト生成、環境変数でオーバーライド可能
  - PIN打刻テンプレートのJS改善 — エラーメッセージの具体化（CSRF検証エラー/セッション切れ）
...
**HANDTEST.md** — 29セクション、220+チェック項目
...
| 28 | セキュリティ改修 Phase 2-3 | 5 | PINハッシュ化、IoT HTTPS、SwitchBot暗号化、IoTレート制限、N+1修正 |
| 29 | 来客予測・CLV・自動発注 | 4 | 来客予測API、CLV分析API、自動発注推奨API、外部データ連携API |
...
| ファイル | 概要 |
|---------|------|
| `IMPLEMENTATION_HISTORY.md` | 実装機能履歴 & テスト項目一覧 (本ファイル) |
| `ROADMAP.md` | 機能拡張ロードマップ (Phase 1-5 + セキュリティTODO) |
| `HANDTEST.md` | 手動テスト項目 (29セクション・220+項目) |
...
| 0065 | attendance_pin ハッシュ化 (max_length 4→128, validator削除) |

---

## ダッシュボード APIView クラス一覧

`booking/views_restaurant_dashboard.py` — 24クラス
...
| 21 | VisitorForecastAPIView | /api/dashboard/visitor-forecast/ | 来客予測 |
| 22 | CLVAnalysisAPIView | /api/dashboard/clv/ | CLV分析 |
| 23 | AutoOrderRecommendationAPIView | /api/dashboard/auto-order/ | 自動発注推奨 |
| 24 | ExternalDataAPIView | /api/dashboard/external-data/ | 外部データ連携 |
