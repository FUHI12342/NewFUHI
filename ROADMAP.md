...
ただし **記述的分析（何が起きた）** のみで、**診断・予測・処方的分析**が欠けている。

### 追加機能

#### 2-1. メニューエンジニアリング・マトリクス (Stars/Plowhorses/Puzzles/Dogs)
...
#### 2-3. 売上予測 (Time Series Forecasting)
...
| 優先度 | Phase | 機能 | 工数目安 | 価値 |
|--------|-------|------|---------|------|
| **P0** | 1 | シフト希望カレンダーUI | 中 | 日常業務の効率化（毎月利用） |
| **P0** | 2-1 | メニューエンジニアリング | 小 | 利益率向上に直結 |
| **P0** | 4-1 | 自動インサイトエンジン | 中 | 経営判断の自動化 |
| **P1** | 2-3 | 売上予測 | 中 | 仕入れ・人員計画の最適化 |
| **P1** | 3-2 | RFM分析 | 小 | リピーター施策の精度向上 |
| **P1** | 3-1 | コホート分析 | 小 | 離脱タイミング特定 |
| **P1** | 4-2 | 経営KPIスコアカード | 小 | 経営状態の一目把握 |
| **P2** | 2-2 | ABC分析 | 小 | 在庫管理効率化 |
| **P2** | 2-4 | 時間帯別売上ヒートマップ | 小 | ピーク最適化 |
| **P2** | 2-5 | 客単価トレンド | 小 | アップセル効果測定 |
| **P2** | 3-3 | マーケットバスケット分析 | 中 | セット提案・メニュー最適化 |
| **P2** | 4-3 | 顧客満足度 (NPS) | 中 | サービス品質の定量化 |
| **P3** | 3-4 | 来客予測 | 中 | スタッフ配置の精度向上 |
| **P3** | 3-5 | CLV | 小 | 長期顧客戦略 |
| **P3** | 4-4 | 自動発注推奨 | 大 | 食品ロス削減 |
| **P3** | 5-4 | 外部データ連携 | 大 | 分析精度向上 |
...
現在地                   目標
  ↓                      ↓
Level 1: 記述的分析      Level 3-4: 予測+処方的分析
「何が起きた？」    →   「何が起きる？」「何をすべき？」

[Level 1] 記述的    ✅ 完了 (売上/来客/シフト/在庫の現状表示)
[Level 2] 診断的    🔨 Phase 2-1, 2-2 (メニュー分析, ABC分析)
[Level 3] 予測的    📋 Phase 2-3, 3-4 (売上予測, 来客予測)
[Level 4] 処方的    📋 Phase 4 (自動インサイト, アクション提案)
```
