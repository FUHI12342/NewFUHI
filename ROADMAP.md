...
> 作成日: 2026-03-12 | 最終更新: 2026-03-13
...
## Phase 4: 処方的分析 — AI経営アドバイザー — 3/4 完了
...
| **P3** | 3-4 | 来客予測 | 中 | スタッフ配置の精度向上 | 📋 未着手 |
| **P3** | 3-5 | CLV | 小 | 長期顧客戦略 | 📋 未着手 |
| **P3** | 4-4 | 自動発注推奨 | 大 | 食品ロス削減 | 📋 未着手 |
| **P3** | 5-4 | 外部データ連携 | 大 | 分析精度向上 | 📋 未着手 |
...
[Level 3] 予測的    ✅ 完了 (売上予測 — 移動平均+信頼区間)  📋 来客予測は未着手
[Level 4] 処方的    ✅ 完了 (自動インサイト, KPIスコアカード, NPS)  📋 自動発注は未着手
```
...
| Chart.js | 可視化 | 既に使用中 |
| Gemini API | 感情分析/インサイト生成 | 既にAIチャットで使用中 |
...
### Phase 2: IoTセキュリティ (HIGH)

| # | 課題 | ファイル | 対応内容 |
|---|------|---------|---------|
| 1 | Setup AP ハードコードパスワード | `pico_device/setup_ap.py` | ランダム生成 or 環境変数化 |
| 2 | API通信 HTTP → HTTPS | `pico_device/config.py` | TLS対応 (Let's Encrypt) |
| 3 | WiFi credentials 平文コミット | `pico_device/secrets.py` | `.gitignore` + secrets管理 |
| 4 | attendance_pin 平文4桁PIN | `booking/models.py` | ハッシュ化 (PBKDF2等) |
| 5 | Watchdogタイマー未実装 | `pico_device/code.py` | WDT追加でハング防止 |
| 6 | DHT22センサープレースホルダー | `pico_device/code.py` | 実装 or 削除 |

### Phase 3: パフォーマンス・信頼性 (MEDIUM)

| # | 課題 | ファイル | 対応内容 |
|---|------|---------|---------|
| 1 | N+1クエリ (ダッシュボードAPI) | `views_restaurant_dashboard.py` | `select_related`/`prefetch_related` |
| 2 | LINE通知リトライなし | `views.py` | 指数バックオフリトライ |
| 3 | IoT APIレート制限なし | `views.py` | `django-ratelimit` or DRF throttle |
