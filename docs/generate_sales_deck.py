...
def create_presentation():
    ...
    add_feature_card(slide, Inches(0.5), Inches(1.2), Inches(5.8), Inches(5.5),
        "オンライン予約システム", [
            ...
            "外部サイト埋め込み対応 — 既存HPにiframeで予約カレンダーを設置可能",
            "無料予約モード — 管理画面からワンクリックで決済スキップ・即確定に切替",
    ...
    admin_screens = [
        ('screenshots/dashboard_sales_vp.png', '売上ダッシュボード',
         '経営状況をひと目で把握',
         [
             '日別・週別・月別・年別の売上推移をグラフで表示',
             '予約KPI（件数・キャンセル率）をリアルタイム集計',
             '総売上・ECショップ・店内メニューの3チャネル別分析',
             'KPIスコアカードで主要指標を一覧表示',
             'ビジネスインサイトが改善機会を自動で提案',
         ]),
        ('screenshots/dashboard_menu_eng_vp.png', '売上データ分析（メニュー分析）',
         'AI分析で利益を最大化',
         [
             'メニューエンジニアリング: Star/Plowhorse/Puzzle/Dogの4象限分類',
             'ABC分析: パレートの法則で売上の80%を生み出す主力商品を特定',
             '売上予測: Prophet AIモデルで14日先の売上を自動予測',
             '時間帯別売上ヒートマップで最適なスタッフ配置を提案',
             '客単価（AOV）推移で価格戦略の効果を検証',
         ]),
        ('screenshots/dashboard_rfm_vp.png', '売上データ分析（顧客分析）',
         'データで顧客理解を深める',
         [
             'RFM分析: 直近性・頻度・金額で顧客を自動セグメント分類',
             'コホート分析: 月別新規顧客のリテンション率を可視化',
             'バスケット分析: 併売パターンからおすすめ商品を発見',
             'AI分析テキスト: 各分析結果をAIが日本語で要約・評価',
             '推奨アクション: 具体的な改善施策をAIが自動提案',
         ]),
        ...
        ('screenshots/iot_sensors_vp.png', 'IoTセンサーモニター',
         '店舗環境をリアルタイム監視',
         [
             '温度・湿度・気圧・ガス濃度をリアルタイム表示',
             'PIR人感センサーで来店客数を自動カウント',
             '異常値検知時にアラート通知（ガス漏れ・高温等）',
             'センサーデータの時系列グラフで傾向を把握',
             'ESP32デバイスの接続状態を常時モニタリング',
         ]),
    ]
    ...
    merits = [
        ...
        ("LINEで予約がラクラク",
         ...
         "💬"),
        ("既存HPにそのまま設置",
         "iframe埋め込みで既存のWordPress等のサイトに\n予約カレンダーをそのまま設置。サイト移行不要です。",
         "🌐"),
        ("無料予約にも対応",
         "イベントや体験予約など決済不要なシーンにも対応。\n管理画面からワンクリックで有料/無料を切替可能。",
         "🎫"),
        ("業務時間を大幅削減",
    ...
    faqs = [
        ...
        ("Q. 既存のホームページに予約機能だけ追加できますか？",
         "A. はい。iframe埋め込み機能で、既存のWordPressサイト等に予約カレンダーをそのまま設置できます。\n"
         "　 サイトの移行やリニューアルは不要です。無料予約モードにも対応しており、イベント等にもご利用いただけます。"),
    ]
