...
- [x] 本番環境 最終QA + インフラ修正 (2026-03-23)
  - ディスク容量確保: 97% → 76% (1.5GB回収)
  - nginx レートリミット緩和: `api` ゾーン 30r/m → 10r/s (503エラー解消)
  - nginx `server_tokens off` 有効化 (バージョン番号非表示)
  - 全ページQAテスト実施 (gstack browse)
  - 多言語テスト (7言語全て正常)
  - レスポンシブテスト (モバイル/タブレット/デスクトップ)
  - セキュリティ評価 9.0/10
  - 詳細: `docs/QA_REPORT_2026-03-23.md`

- [x] 残課題対応 (2026-03-23)
  - ショップ商品画像: プレースホルダー検出 + 「サンプル」バッジ表示 (`_is_placeholder_image`)
  - CSP + Permissions-Policy ヘッダー追加 (nginx snippets/security-headers.conf)
  - Tailwind CDN → ビルド済みCSS移行 (37KB, `npm run css:build`)
  - IoTセンサー: デバイスローディング改善 (自動リトライ3回 + 手動リトライボタン + エラーメッセージ)
  - テスト 4件追加 (全1315件 pass)
